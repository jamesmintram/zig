...
fn airDbgVarPtr(self: *Self, inst: Air.Inst.Index) !void {
    const pl_op = self.air.instructions.items(.data)[inst].pl_op;
    const operand = pl_op.operand;
    const ptr_ty = self.air.typeOf(operand);
    const mcv = try self.resolveInst(operand);
    const name = self.air.nullTerminatedString(pl_op.payload);
    try self.genVarDbgInfo(ptr_ty.childType(), mcv, name);
    return self.finishAir(inst, .dead, .{ operand, .none, .none });
}

fn airDbgVarVal(self: *Self, inst: Air.Inst.Index) !void {
    const pl_op = self.air.instructions.items(.data)[inst].pl_op;
    const operand = pl_op.operand;
    const ty = self.air.typeOf(operand);
    const mcv = try self.resolveInst(operand);
    const name = self.air.nullTerminatedString(pl_op.payload);
    try self.genVarDbgInfo(ty, mcv, name);
    return self.finishAir(inst, .dead, .{ operand, .none, .none });
}
...
fn genVarDbgInfo(
    ...
    switch (mcv) {
        ...
        else => return self.fail("TODO generate debug info for {}", .{mcv}),
    }
}
